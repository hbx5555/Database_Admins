    ...
    },
    {
      ...
      component: ({ rowData, setRowData, focus }: { rowData: DealRow; setRowData: (r: DealRow) => void; focus: boolean }) => {
        ...
        return (
          <div style={{ padding: '0 8px', display: 'flex', alignItems: 'center', height: '100%' }}>
            {rowData.status
              ? <span style={{ display: 'inline-block', padding: '2px 10px', borderRadius: 'var(--radius-pill)', backgroundColor: DEAL_STATUS_COLORS[rowData.status].bg, color: DEAL_STATUS_COLORS[rowData.status].text, fontSize: 13, fontFamily: 'var(--font-captions)', fontWeight: 500, lineHeight: '20px', whiteSpace: 'nowrap' }}>{rowData.status}</span>
              : <span style={{ color: 'var(--foreground-secondary)' }}>—</span>}
          </div>
        )
      },
